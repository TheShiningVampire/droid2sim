...
|---|---:|
| Other source documents/manifests | 1,045,744 |
| DROID 100-episode RLDS sample | 2,192,615,094 |
| Menagerie Panda and Robotiq assets | 40,900,690 |
| Published DROID calibration/ID maps | 358,986,847 |
| Rejected mug raw data (archived under data/rejected_mug_raw) | 13,492,357 |
...
`a502d4783a34879137a77bf74fff2677db9fb4fed89069c0a5ba9b0e9171ff4b`
...
|---|---|---:|---:|---|---|
| 1 | FAIL | 13.219 | 12 | Initial marker was too long for the bowl interior; up to 13.2 mm initial wall overlap. It rotated before grasping and stayed in the bowl. | [Compare](artifacts/iteration1/side_by_side.mp4) · [Sim](artifacts/iteration1/sim_ext1.mp4) |
| 2 | PASS | 4.227 | 47 | Shortened marker 140→120 mm, radius 7.5→8.4 mm, widened lower bowl 53→66 mm, shifted marker y by −11 mm and reversed cap. No initial overlap. Marker lifted and released onto table, but rotates during capture. | [Compare](artifacts/iteration2/side_by_side.mp4) · [Sim](artifacts/iteration2/sim_ext1.mp4) |
| 3 | FAIL | 0.371 | 9 | Applied stiffer 6D marker contacts. Penetration decreased sharply; marker was pushed out of the pinch and stayed in bowl. | [Compare](artifacts/iteration3/side_by_side.mp4) · [Sim](artifacts/iteration3/sim_ext1.mp4) |
| 4 | FAIL | 0.513 | 11 | Retained stiff contacts; moved bowl and marker (+14,−8,0) mm. Grasp still failed and bowl displacement increased; visually worse. | [Compare](artifacts/iteration4/side_by_side.mp4) · [Sim](artifacts/iteration4/sim_ext1.mp4) |
| 5 | FAIL | 0.721 | 11 | Restored iteration-2 geometry and native pad response; stiffened marker–bowl pairs only; disabled hard shadows. Grasp still failed. Five-iteration limit reached. | [Compare](artifacts/iteration5/side_by_side.mp4) · [Sim](artifacts/iteration5/sim_ext1.mp4) |
...
## Follow-up: pipeline explanation and SysID diagnostics

After the initial five iterations, the user requested a short explanation and asked about SysID. [pipeline_explainer.mp4](pipeline_explainer.mp4) is a 42-second silent, captioned video showing the real-to-sim inputs, method, failures and limitations. Episode excerpts run at approximately 2× speed for explanation; the actual replay/comparison files remain at 15 Hz.

No physical SysID has been performed. An **offline first-order joint command-response surrogate** was subsequently fitted to 120 training samples and assessed on 60 held-out transitions. It improves one-step prediction over a hold-state baseline, but does not identify physical masses, friction, contact stiffness or directly usable MuJoCo gains. No fitted parameter was applied to the scene or controller. Details and limitations: [SYSID.md](SYSID.md).

Offline exterior-1 teal-cap tracking adds an objective visual measure: 168 matched frames, mean cap-position error 7.82 px, transport mean 13.43 px, final cap error 2.76 px. Subtracting initial image offset still leaves 10.63 px mean transport error. This confirms motion/orientation disagreement beyond constant image alignment. These are appearance-centroid measurements, not calibrated 3D ground truth. [Tracking review](artifacts/measurements/tracking_review.jpg) and JSON measurements are retained. All five original physics runs and failures remain unchanged; no sixth scene iteration was made.

## Code organization and publication

The maintained implementation now lives in `droid2sim/`, separated into data, simulation, visualization and evaluation modules. Run `python -m droid2sim --help` for the shared CLI. Root replay/check/comparison wrappers remain compatible. Supporting analysis and media commands live under `scripts/`; historical one-off scripts are under `archive/`. See [architecture](docs/ARCHITECTURE.md).

All five comparisons have full-length GIF previews in [the README](README.md). Failed videos and their original metrics remain available. Reorganization did not add a physics iteration: regenerated robot and scene XML are byte-identical, all six comparison overlays are pixel-identical, and trajectory/archived result hashes are unchanged. Six regression tests, formatting/lint checks and the full local provenance audit pass. Dataset downloads, virtual environment, vendor meshes and NumPy arrays are excluded from Git; setup commands recreate required local inputs.
